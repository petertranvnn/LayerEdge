...
if ! command -v cargo &> /dev/null; then
    ...
    if ! command -v cargo &> /dev/null; then
        echo -e "${RED}❌ Không thể cài Rust. Vui lòng kiểm tra kết nối hoặc chạy thủ công: curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh${NC}"
        exit 1
    fi
fi
...
echo -e "${GREEN}Danh sách phiên screen: screen -ls${NC}"
